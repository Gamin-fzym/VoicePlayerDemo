...
- (void)playAudioWithUrl:(NSString *)url {
    ...
    if (result) {
        ...
        NSData *amrData = [NSData dataWithContentsOfURL:[NSURL URLWithString:url]];
        NSData *wavData = DecodeAMRToWAVE(amrData);
        _player = [[AVAudioPlayer alloc] initWithData:wavData error:&playError];
...
- (void)createRecorder {
    ...
    // 录音的质量
    [settingDic setValue:[NSNumber numberWithInt:AVAudioQualityLow] forKey:AVEncoderAudioQualityKey];
    // 大端还是小端 是内存的组织方式
    [settingDic setValue:[NSNumber numberWithBool:NO] forKey:AVLinearPCMIsBigEndianKey];
    // 采样信号是整数还是浮点数
    [settingDic setValue:[NSNumber numberWithBool:NO] forKey:AVLinearPCMIsFloatKey];

    // 录音文件最终地址
    _recordURL = [CDPAudioRecorder getAudioRecordFilePathWithMark:NO];
    NSError *error = nil;
...
// 获得录音文件最终地址 mark:是否强制创建路径
+ (NSURL *)getAudioRecordFilePathWithMark:(BOOL)mark {
    NSString *path = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject];
    NSString *filePath = [path stringByAppendingPathComponent:@"CDPAudioFiles/CDPAudioRecord.caf"];
    // 判断是否存在,不存在则创建
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath:filePath] || mark) {
